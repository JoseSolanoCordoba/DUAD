...
def verify_student_existence(students_data_list, student_object):
    for student in students_data_list:
        if (student_object.name == student.name) and (student_object.section == student.section):
            return True
...
class Student():
    def __init__(self, name, section, spanish_grade, english_grade, social_studies_grade, science_grade):
        ...
        self.average = 0
student_1 = Student("andres","11c", 100, 100, 100, 100)
student_2 = Student("jess","11c", 10, 100, 10, 100)
student_3 = Student("juan","11c", 100, 100, 10, 100)
student_4 = Student("jose","11c", 10, 10, 100, 10)
...
def request_students_data(students_grades_info, getting_data):
    ...
    if getting_data =="1":
        ...
        students_grades_info = [student_1, student_2, student_3, student_4]
        while(True):
            addingmore = input(f"Do you still want to add student data? 1:yes, any key for exit\n")
            if addingmore!="1":
                break

            """student_data = Student(request_name(), request_section(), request_spanish_grade(), request_english_grade(), request_social_studies_grade(), request_science_grade())
